...
TEST(TestCtrvModel, NextStateRotationAndTranslation)
{
  ...
  const auto next_state{ cp::nextState(state, 0.5_s) };
  const cp::CtrvState expected_state{ 0.479425539_m, 0.122417438_m, 1_mps, cp::Angle(0.5_rad), 1_rad_per_s };
...
TEST(TestCtrvModel, NextStateStochastic)
{
  ...
  const cp::CtrvState expected_state{ 0.604425539_m, 0.122417438_m, 1.5_mps, cp::Angle(0.625_rad), 1.5_rad_per_s };
...
TEST(TestCtrvModel, Equality)
{
  ...
  const cp::CtrvState state1{ 1.23_m, 2.435_m, 5544_mps, cp::Angle(34656.6543_rad), 595633.555_rad_per_s };
  const cp::CtrvState state2{ 1.2_m, 20.45_m, 4_mps, cp::Angle(34656.65435_rad), 5953.55_rad_per_s };
  const cp::CtrvState state3{ 1_m, 2_m, 4_mps, cp::Angle(3_rad), 1.000000000000000000000000001_rad_per_s };
