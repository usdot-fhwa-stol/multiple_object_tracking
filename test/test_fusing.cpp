...
/**
 * Test the generateWeight function
 */
...
/**
 * Test the computeCovarianceIntersection function using purely Eigen matrices and vectors
 */
...
TEST(TestFusing, CtrvTracksAndDetections)
{
  ...
  std::vector<cp::TrackVariant> tracks{
    cp::CtrvTrack{ .timestamp{ units::time::second_t{ 0 } },
                   ...
                   .covariance{ cp::CtrvStateCovariance{ { -0.71124, 0.25895, 0.07334, 0.90743, -0.90324 },
    ...
    cp::CtrvTrack{ .timestamp{ units::time::second_t{ 0 } },
                   ...
                   .covariance{ cp::CtrvStateCovariance{ { -0.85640, 0.00882, 0.04030, 0.13055, -0.16262 },
    ...
    cp::CtrvTrack{ .timestamp{ units::time::second_t{ 0 } },
                   ...
                   .covariance{ cp::CtrvStateCovariance{ { 0.30846, 0.87769, 0.73339, 0.32553, 0.63853 },
  ...
  std::vector<cp::DetectionVariant> detections{
    cp::CtrvDetection{
        ...
        .state{ cp::CtrvState{ 10.72978_m, 12.57061_m, 13.00000_mps, cp::Angle(2.65044_rad), 15.00000_rad_per_s } },
        .covariance{ cp::CtrvStateCovariance{ { 0.88963, 1.35620, 0.95801, -0.73548, -0.62768 },
    ...
    cp::CtrvDetection{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtrvState{ 6.45785_m, 6.65389_m, 8.00000_mps, cp::Angle(1.88242_rad), 10.00000_rad_per_s } },
        .covariance{ cp::CtrvStateCovariance{ { 0.62623, 0.06581, 0.50370, -0.34803, 0.25890 },
    ...
    cp::CtrvDetection{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtrvState{ 1.16422_m, 1.65312_m, 3.00000_mps, cp::Angle(2.46081_rad), 5.00000_rad_per_s } },
        .covariance{ cp::CtrvStateCovariance{ { 1.83854, 0.34738, 0.41240, -2.36064, 0.46715 },
  ...
  std::vector<cp::TrackVariant> expected_tracks{
    cp::CtrvTrack{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtrvState{ 1.05749_m, 2.04737_m, 3.03512_mps, cp::Angle(3.86391_rad), 4.90450_rad_per_s } },
        .covariance{ cp::CtrvStateCovariance{ { -0.25576, 0.12667, -0.18088, 0.75883, -0.44233 },
    ...
    cp::CtrvTrack{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtrvState{ 4.50541_m, 5.23887_m, 1.77684_mps, cp::Angle(0.48295_rad), 4.59700_rad_per_s } },
        .covariance{ cp::CtrvStateCovariance{ { 0.01323, 0.01681, 0.04270, 0.00542, 0.02767 },
    ...
    cp::CtrvTrack{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtrvState{ 8.27323_m, 9.02424_m, 11.10270_mps, cp::Angle(3.92098_rad), 16.39624_rad_per_s } },
        .covariance{ cp::CtrvStateCovariance{ { 0.08726, 0.06046, 0.07614, -0.10960, 1.43314 },
  ...
  // Check that function returns expected value
  EXPECT_TRUE(cp::utils::almostEqual(result_tracks, expected_tracks));
}
...
TEST(TestFusing, CtraTracksAndDetections)
{
  ...
  std::vector<cp::TrackVariant> tracks{
    cp::CtraTrack{
        ...
        .covariance{ cp::CtraStateCovariance{ { 0.66016, 0.67280, 0.87234, -0.04035, 0.81610, 0.40492 },
    ...
    cp::CtraTrack{
        ...
        .covariance{ cp::CtraStateCovariance{ { -0.49361, -0.65374, 0.92303, 0.15896, -0.47469, 0.18736 },
    ...
    cp::CtraTrack{
        ...
        .covariance{ cp::CtraStateCovariance{ { -0.28565, -0.96979, -0.46169, 0.43703, -0.88999, -0.05841 },
  ...
  std::vector<cp::DetectionVariant> detections{
    cp::CtraDetection{
        .timestamp{ units::time::second_t{ 0 } },
        .state{
            cp::CtraState{ 12.68893_m, 12.94951_m, 24.0_mps, cp::Angle(4.07965_rad), 17.0_rad_per_s, 18.0_mps_sq } },
        .covariance{ cp::CtraStateCovariance{ { 3.32452, -0.90824, -0.52113, -3.57288, -0.81460, 0.34692 },
    ...
    cp::CtraDetection{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtraState{ 7.46094_m, 8.81804_m, 15.0_mps, cp::Angle(2.93363_rad), 11.0_rad_per_s, 12.0_mps_sq } },
        .covariance{ cp::CtraStateCovariance{ { 1.00910, -0.37564, 0.08011, -1.56304, -0.51999, 0.71663 },
    ...
    cp::CtraDetection{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtraState{ 1.29581_m, 1.59029_m, 6.0_mps, cp::Angle(2.18031_rad), 5.0_rad_per_s, 6.0_mps_sq } },
        .covariance{ cp::CtraStateCovariance{ { 3.32452, -0.90824, -0.52113, -3.57288, -0.81460, 0.34692 },
                                              { -0.90824, 2.57238, -0.17246, 2.27956, 0.13771, -0.69168 },
                                              { -0.52113, -0.17246, 0.56438, 0.55992, 0.07005, 0.13371 },
                                              { -3.57288, 2.27956, 0.55992, 8.25950, 1.02821, 0.45153 },
  ...
  std::vector<cp::TrackVariant> expected_tracks{
    cp::CtraTrack{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtraState{ 0.23685_m, 3.65177_m, 6.13595_mps, cp::Angle(3.98704_rad), 5.43771_rad_per_s,
                               3.84887_mps_sq } },
        .covariance{ cp::CtraStateCovariance{ { 1.73333, 0.78889, -0.52390, 0.13104, 1.24063, -0.28648 },
    ...
    cp::CtraTrack{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtraState{ 2.70064_m, 21.79559_m, 20.15401_mps, cp::Angle(2.00117_rad), 19.25853_rad_per_s,
                               6.63957_mps_sq } },
        .covariance{ cp::CtraStateCovariance{ { 0.88278, 0.34604, -1.16504, -2.15773, -0.93873, 0.12978 },
    ...
    cp::CtraTrack{
        .timestamp{ units::time::second_t{ 0 } },
        .state{ cp::CtraState{ 8.94162_m, 18.04843_m, 18.98816_mps, cp::Angle(1.32344_rad), 15.33205_rad_per_s,
                               23.22585_mps_sq } },
        .covariance{ cp::CtraStateCovariance{ { -0.86394, -0.09628, -0.28560, 1.39213, -1.39202, 0.13592 },
...
TEST(TestFusing, UnmatchedAssociations)
{
  ...
  // Declaring initial values
  cp::AssociationMap associations{ { "track1", { "detection4" } },
                                   { "track2", { "detection5" } },
                                   { "track3", { "detection6" } } };
