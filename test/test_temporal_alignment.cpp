...
TEST(TestTemporalAlignment, CtrvDetection)
{
  ...
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_detection_immutable.state, 4),
                                     cp::utils::roundToDecimalPlace(expected_state, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_detection_immutable.covariance, 4),
                                     cp::utils::roundToDecimalPlace(expected_covariance, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(result_detection_immutable.timestamp.value(), time_step.value()));
}
...
TEST(TestTemporalAlignment, CtrvDetectionFiveSeconds)
{
  ...
  TestDetection detection{ .timestamp{ units::time::second_t{ 0 } },
                           ...
                           .covariance{ cp::CtrvStateCovariance{ { 0.0043, -0.0013, 0.0030, -0.0022, -0.0020 },
                                                                 ...
                                                                 { -0.0020, 0.0060, 0.0008, 0.0100, 0.0123 } } } };
  // Expected values
  const cp::CtrvState expected_state{ 7.67240_m, 10.08887_m, 2.20490_mps, cp::Angle(2.26550_rad), 0.35280_rad_per_s };
...
TEST(TestTemporalAlignment, CtraDetection)
{
  ...
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_detection.state, 4),
                                     cp::utils::roundToDecimalPlace(expected_state, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_detection.covariance, 5),
  ...
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_detection_immutable.state, 4),
                                     cp::utils::roundToDecimalPlace(expected_state, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_detection_immutable.covariance, 5),
                                     cp::utils::roundToDecimalPlace(expected_covariance, 5)));
  EXPECT_TRUE(cp::utils::almostEqual(result_detection_immutable.timestamp.value(), time_step.value()));
}
...
TEST(TestTemporalAlignment, CtraDetectionFiveSeconds)
{
  ...
  TestDetection detection{ .timestamp{ units::time::second_t{ 0 } },
                           ...
                           .covariance{ cp::CtraStateCovariance{ { 0.0043, -0.0013, 0.0030, -0.0022, -0.0020, 0.5 },
                                                                 ...
                                                                 { 0.5, 0.123, -0.34, 0.009, 0.0021, -0.8701 } } } };
  // Expected values
  const cp::CtraState expected_state{ 8.92670_m,         21.12001_m,    7.20490_mps, cp::Angle(2.26550_rad),
                                      0.35280_rad_per_s, 1.00000_mps_sq };
  const cp::CtraStateCovariance expected_covariance{
    { 3272.17529, -2753.25513, -1620.65393, -2.53488, -0.43595, -324.23853 },
    { -2753.25513, 34524.34766, 15688.62109, -8.90359, -1.52473, 3138.59009 },
    { -1620.65393, 15688.62109, 7227.46875, 0.10220, 0.01130, 1445.83264 },
    { -2.53488, -8.90359, 0.10220, 0.41730, 0.07150, 0.01950 },
    { -0.43595, -1.52473, 0.01130, 0.07150, 0.01230, 0.00210 },
    { -324.23853, 3138.58960, 1445.83240, 0.01950, 0.00210, 289.23453 }
  };
...
TEST(TestTemporalAlignment, CtrvTrack)
{
  ...
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_track_immutable.state, 4),
                                     cp::utils::roundToDecimalPlace(expected_state, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_track_immutable.covariance, 4),
                                     cp::utils::roundToDecimalPlace(expected_covariance, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(result_track_immutable.timestamp.value(), time_step.value()));
}
...
TEST(TestTemporalAlignment, CtraTrack)
{
  ...
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_track.state, 4),
                                     cp::utils::roundToDecimalPlace(expected_state, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_track.covariance, 5),
                                     cp::utils::roundToDecimalPlace(expected_covariance, 5)));
  EXPECT_TRUE(cp::utils::almostEqual(result_track.timestamp.value(), time_step.value()));
  ...
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_track_immutable.state, 4),
                                     cp::utils::roundToDecimalPlace(expected_state, 4)));
  EXPECT_TRUE(cp::utils::almostEqual(cp::utils::roundToDecimalPlace(result_track_immutable.covariance, 5),
                                     cp::utils::roundToDecimalPlace(expected_covariance, 5)));
  EXPECT_TRUE(cp::utils::almostEqual(result_track_immutable.timestamp.value(), time_step.value()));
};
